...
export default async (req, context) => {
  ...
  if (!token || !repo) return Response.json({ error: 'Server not configured (GITHUB_TOKEN/GITHUB_REPO)' }, { status: 500 });

  const { texts = {}, images = {} } = await req.json();
  const idOk = s => /^[a-z0-9-]+$/.test(s);

  // load current files we need to patch
  ...
  for (const [id, dataUrl] of Object.entries(images)) {
    ...
    // remove stale exports so CI regenerates them from the new upload
  ...
  if (!treeEntries.length) return Response.json({ ok: true, note: 'nothing to save' });
